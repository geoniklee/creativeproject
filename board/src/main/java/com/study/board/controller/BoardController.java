...
public class BoardController {
    ...
    @PostMapping("/board/output")                       /// 해당 매개변수는 Entity를 통해 가져온다. //file 업로드를 위해서 MultipartFile 추가
    public String boardoutput(Board board, Model model, MultipartFile file, MultipartFile file2) throws Exception
    {
        boardService.write(board,file, file2);
        model.addAttribute("message","글 작성이 완료되었습니다.");
        model.addAttribute("searchUrl","/board/list");      // /board/list를 통해서 모델에 담겨서 message에 담겨진다.
